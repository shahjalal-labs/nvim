...
local function intelligentGitPushOneLocAvailable(git_root)
	...
	vim.fn.jobstart({ "git", "-C", git_root, "add", "." }, {
		on_exit = function()
			...
			if insertions + deletions < 1 then
				vim.schedule(function()
					vim.notify("⏸️ Skipped push — changes less than 1 LOC: " .. git_root, vim.log.levels.INFO)
				end)
				return
			end
